...
class MedicamentsPage extends StatefulWidget {
  ...
  const MedicamentsPage({super.key, required this.patientId,  this.typeFilter , this.readOnly=false });
...
class _MedicamentsPageState extends State<MedicamentsPage> {
  ...
  Future<void> playSound() async {
    await player.play(
      AssetSource('mixkit-quick-positive-video-game-notification-interface-265.wav'),
    );
  }
  ...
  setState(() {
    if (widget.typeFilter == null) {
      ...
      meds = all.where((m) =>
          ...
          widget.typeFilter!.toLowerCase()).toList();
    }
  });
...
await sendPatientNotification(
  ...
  body:
      "Le docteur a ajouté ${name} dans Médicaments",
);
...
final med = meds.firstWhere(
  ...
  body:
      "Le docteur a supprimé ${med["name"]} dans Médicaments",
);
...
"dateFin": Timestamp.fromDate(
  ...
  body:
      "Le docteur a modifié ${name} dans Médicaments",
);
  ...
  if (picked != null) {
    controller.text = "${picked.year}-${picked.month.toString().padLeft(2,'0')}-${picked.day.toString().padLeft(2,'0')}"; // yyyy-MM-dd
  }
}
//hedhou ui
...
GestureDetector(
  ...
  child: AbsorbPointer(
    child: buildField(dateDebut, "Date début", Icons.calendar_today),
  ),
),
...
GestureDetector(
  ...
  TextEditingController quantite = TextEditingController(text: med["quantite"]);
...
TextEditingController dateDebut =
  ...
  child: Column(
    ...
    children: [
      ...
      Row(
        children: [
          CircleAvatar(
            backgroundColor: getColorByType(med["type"]).withOpacity(0.2),
            child: Icon(
      ...
      infoRow(Icons.access_time, "Heure", med["time"], getColorByType(med["type"])),

infoRow(Icons.wb_sunny, "Moment", med["moment"], getColorByType(med["type"])),

infoRow(Icons.repeat, "Fois/jour", med["fois"], getColorByType(med["type"])),

infoRow(Icons.format_list_numbered, "Quantité", med["quantite"], getColorByType(med["type"])),

infoRow(Icons.calendar_today, "Durée", med["duree"], getColorByType(med["type"])),
infoRow(
Icons.date_range,
"Début",
