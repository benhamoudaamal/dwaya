...
class TypesMedicamentsPage extends StatefulWidget {
  ...
  const TypesMedicamentsPage({super.key,  required this.patientId ,this.readOnly = false,  });
...
List<Map<String, dynamic>> types = [
      {"name": "Gélule", "icon": Icons.medication, "color": Colors.blue, "fav": false, "count":0},
      {"name": "Comprimé", "icon": Icons.circle, "color": Colors.green , "fav": false, "count":0},
      {"name": "Sirop", "icon": Icons.local_drink, "color": Colors.orange , "fav": false, "count":0},
      {"name": "Crème", "icon": Icons.spa, "color": Colors.pink , "fav": false, "count":0},
      {"name": "Injection", "icon": Icons.vaccines, "color": Colors.red , "fav": false, "count":0},
      {"name": "Gouttes", "icon": Icons.opacity, "color": Colors.cyan , "fav": false, "count":0},
      {"name": "Patch", "icon": Icons.healing, "color": Colors.purple , "fav": false, "count":0},
      {"name": "Inhalateur", "icon": Icons.air, "color": Colors.teal , "fav": false, "count":0 },
      {"name": "Gel", "icon": Icons.water_drop, "color": Colors.lightBlue , "fav": false, "count":0},
      {"name": "Sachet", "icon": Icons.inventory_2, "color": Colors.amber , "fav": false, "count":0},
      {"name": "Spray", "icon": Icons.sanitizer, "color": Colors.indigo , "fav": false, "count":0},
    ];
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: widget.readOnly ? null : () async {
                  ...
                  await FirebaseFirestore.instance
                      .collection("medicine_types")
                      .doc(controller.text.toLowerCase().replaceAll(" ", "_"))
                      .set({
...
int totalFavs() => types.where((e) => e["fav"] == true).length;
 ...
 void showStatsDialog() {
  ...
  Future<String> translateText(String text) async {
    var result = await
translator.translate(text, to: selectedLanguage);
    return result.text;
  }
 ...
 Future<void> playSounds(String name) async {
  ...
  if (sounds.containsKey(key)) {
    String file =
        sounds[key]![selectedLanguage] ?? "click.mp3";
  ...
  showDialog(
    ...
    builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: widget.readOnly ? null : () async {
            String deletedName = types[index]["name"]; // 🔥 احفظ الاسم قبل الحذف
...
Widget build(BuildContext context) {
  ...
  var filtered = types.where((e) =>
    e["name"].toLowerCase().contains(searchText)).toList();
...
bool isFav =
    favBy.contains(widget.patientId);
//bool isFav = (type["favBy"] ?? []).contains(widget.patientId);
  ...
  if (isFav) {
    ...
    await FirebaseFirestore.instance
        ...
        .update({
      "favBy":
          FieldValue.arrayRemove([widget.patientId])
    });
   ...
   //   type["favBy"].remove(widget.patientId);
    ...
    await FirebaseFirestore.instance
        ...
        .update({
      "favBy":
          FieldValue.arrayUnion([widget.patientId])
    });
  }
   // setState(() {
